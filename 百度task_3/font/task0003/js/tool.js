define(function(){
    return {
        init : function () {
            ...
            if (window.classArray.length == 0) {// 如果初次访问应用
               ...
               // alert('666')
                ...
                var option = {
                        class : 'alert',
                        inner : '<p class="first-line">欢迎来到Todu,在这里，你可以制定自己的计划，还在担心没有地方方便的记录下你的play吗？开始你的Todu吧~</p>'
                        };
                alertBtn.innerHTML = "下一步" ;
                self.maskHandler(option);
                alertBtn.onclick = function () {
                    alertBtn.innerHTML = "Todu it!";
                       option = {
                        class : 'alert',
        ...
        storageOprate : {
            ...
            window.localStorage.setItem('class',JSON.stringify(window.classArray));// 存储class，把JavaScript值序列化成JSON字符串。
            },
            ...
            a();
            window.localStorage.setItem('task',JSON.stringify(window.taskArray));// 存储task，把JavaScript值序列化成JSON字符串。
            },
            setStorage: function (id,obj) {// options修改的参数
               var array = window.taskArray,i,len;
                ...
                function arraySort () {// 根据tasK日期重排序数组
                    function compare (value1,value2) {
                        var value1 = new Date(value1.date).getTime()/3600/1000;
                        var value2 = new Date(value2.date).getTime()/3600/1000;
                        if (value1 > value2) {
            ...
            deleteClass : function (id) {
                var task = window.taskArray;
                var array = window.classArray;
                var newClassArray = [];
                var newTaskArray = [];
                var i,j,len,leng;
                for (i =0,len = array.length; i < len ; i++ ) {
                    if (array[i].id == id || array[i].belong == id) {  
                    } 
                    else {
                      newClassArray.push(array[i]);
                    }
                }
                window.classArray = newClassArray;// 更新classArray
                array = window.classArray;
                for ( i =0,len = task.length; i < len ; i++) {
                    for (j = 0,leng = array.length; j < len ; j++ ) {
                         if ( task[i].belong == array[j].id) {// 如果这个任务属于某个分类
                            newTaskArray.push(task[i]);
                         }
                    }
                }
        ...
        deleteNumberHandler: function(id) {// id为子分类的id
           ...
           window.localStorage.setItem('class',JSON.stringify(window.taskArray));
        },
